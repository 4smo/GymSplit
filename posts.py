...
def update_post(title, content_days, tag, user_id, post_id):
    sql = "UPDATE posts SET title = ?, content_day1 = ?, content_day2 = ?, content_day3 = ?, content_day4 = ?, content_day5 = ?, content_day6 = ?, content_day7 = ?, tag = ?, user_id = ? WHERE id = ?"
    params = [title] + content_days + [tag, user_id, post_id]
    db.execute(sql, params)
